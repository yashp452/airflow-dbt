...
def create_glue_job(
    ...
    **kwargs
    ...
    # Add execution date from templates_dict if provided
    if templates_dict and 'execution_date' in templates_dict:
        arguments['--date'] = templates_dict['execution_date']
    ...
    # Spark ETL job - original configuration
    spark_configurations = [
        ...
        f'spark.sql.catalog.glue_catalog.warehouse=s3://{s3_bucket}/iceberg-warehouse/',
    ...
    # Only create/update job if it doesn't exist
    job_exists = False
    try:
        glue_client.get_job(JobName=job_name)
        job_exists = True
        print(f"Job '{job_name}' already exists. Skipping update - will run with new parameters.")
    except ClientError as e:
